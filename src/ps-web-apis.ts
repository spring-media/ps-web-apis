...
export type WaitingRoomQueue =
  | WaitingRoomQueueDefault
  | "auth"
  | "checkout"
  | "lefty-in-app-purchase"
  | "logora";
...
export type RegisterIframeMessageListener = (
  eventtype: string,
  listener: (event: any, iframe: HTMLIFrameElement) => void
) => void;
...
export interface WhoamiV1 {
   ...
   */
  ensureUserHasFirstNameAndLastName(
    skippable: boolean,
    title?: string,
    text?: string
   ...
   */
  getServicePassport(
    service: "logora" | string
  ): Promise<ServicePassportResponse>;
}
...
export interface AbV1 {
  userInTestGroupForFeature: (
    key: string
  ) => { canSeeFeature: boolean; testGroup: "A" | "B" };
}
